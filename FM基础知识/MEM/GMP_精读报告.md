...
- **计算成本爆炸**：在 Transformer 架构中，对 \(H\) 步历史做自注意力的复杂度是 \(\mathcal{O}(H^2)\)，随着历史长度增加，训练和推理成本急剧上升。
...
训练时，对真实动作 \(A^0_{t:t+h}\) 逐步加高斯噪声 \(\epsilon \sim \mathcal{N}(0, I)\)，用去噪网络 \(\varphi_\theta\) 预测原始动作，损失函数为：
...
$$\{A_{t-nh:t-(n-1)h}, \cdots, A_{t-h:t}\}$$

每个动作块对应一个采样的图像观测（高频帧存在大量冗余，因此每块只采样一帧）：

$$I_{t-nh:h:t} = \{I_{t-nh}, I_{t-(n-1)h}, \cdots, I_{t-h}\}$$
...
如果直接把历史 token 与当前 token 在序列维度上拼接，用标准**自注意力**处理，计算复杂度为 \(\mathcal{O}(H^2)\)，\(H\) 是历史 token 总数。
...
- **Query** 来自 DiT 的当前 hidden states：\(\mathbf{z}_{t:t+h} \in \mathbb{R}^{h \times d}\)
- **Key / Value** 来自缓存的历史 chunk（每个 chunk = 1 个聚合图像 token + \(h\) 个动作 token）

通过交叉注意力，Query attend 到历史中最相关的信息，输出历史上下文向量 \(\mathbf{h}_{t:t+h} \in \mathbb{R}^{h \times d}\)。

**关键优势**：Cross-Attention 的复杂度为 \(\mathcal{O}(h \times H)\)，对 \(H\) 是**线性**而非二次。推理时，如果 Gate 关闭，则完全跳过历史注意力，推理时间与无历史策略相同。
...
>
![GMP 方法架构](./images/gmp_method.jpg)
>
...
给定当前观测 \(I_t\) 和本体感受 \(P_t\)，用一个 MLP \(\phi\)（含 Sigmoid 激活）生成二进制门值 \(\mu_t\)：

$$\mu_t = \mathbf{1}\{ \sigma(\phi(I_t, P_t)) > 0.5 \} \in \{0, 1\}$$

门值 \(\mu_t\) 作用在历史 Cross-Attention 的输出 \(\mathbf{h}_{t:t+h}\) 上，决定是否将历史信息融入当前输出：

$$\bar{\mathbf{z}}_{t:t+h} = \mu_t \cdot \mathbf{h}_{t:t+h} + \mathbf{z}_{t:t+h}$$

- \(\mu_t = 0\)：历史信息被完全阻断，等同于无记忆策略
- \(\mu_t = 1\)：历史信息被融合进策略输出
...
> 四步校准：① 在 \(\mathcal{D}_{\text{train}}\) 上训练 Gate 常闭/常开两个策略；② 在 \(\mathcal{D}_{\text{val}}\) 上采样 \(N\) 条轨迹并计算各时刻动作预测误差；③ 对比误差比值生成二值标签 \(\mu_t\)；④ 冻结 Gate 权重，在完整数据集上重训策略

**Step 1**：将数据集划分为 \(\mathcal{D}_{\text{train}}\) 和 \(\mathcal{D}_{\text{val}}\)。

**Step 2**：在 \(\mathcal{D}_{\text{train}}\) 上分别训练两个策略：
- \(\pi\)：Memory Gate 始终关闭（\(\mu_t = 0\)，无记忆）
- \(\pi_{\text{mem}}\)：Memory Gate 始终打开（\(\mu_t = 1\)，强制记忆）

**Step 3**：在 \(\mathcal{D}_{\text{val}}\) 上对两个策略各采样 \(N\) 条轨迹，计算每个时间步 \(t\) 的动作预测误差：
- \(\delta_t\)：无记忆策略 \(\pi\) 的误差
- \(\delta_t^{\text{mem}}\)：有记忆策略 \(\pi_{\text{mem}}\) 的误差

**Step 4**：引入比率阈值 \(\theta\)，生成门标签：

$$\mu_t = \begin{cases} 1 & \text{if } \delta_t \geq \theta \cdot \delta_t^{\text{mem}} \quad (\text{记忆明显有帮助}) \\ 0 & \text{otherwise} \end{cases}$$

即：如果某个时间步**无记忆时的误差明显大于有记忆时的误差**，说明记忆在此刻是必要的。

**Step 5**：用 Binary Cross Entropy Loss 单独训练 Memory Gate MLP \(\phi\)。

**Step 6**：冻结 Memory Gate 参数，在完整数据集（\(\mathcal{D}_{\text{train}} + \mathcal{D}_{\text{val}}\)）上重新训练策略，得到最终 **Gated Memory Policy** \(\pi_{\text{gated}}\)。

#### 3.3.4 门控的运行特点

实验发现：
- 在 Markov 任务上，门几乎始终关闭（\(\mu = 0\)），因为记忆不影响性能
...
![GMP 注意力可视化](./images/gmp_attention.jpg)
...
2. **Cross-Attention 替代自注意力**：将历史建模从 \(\mathcal{O}(H^2)\) 降为 \(\mathcal{O}(h \times H)\)，配合 KV Cache 实现高效历史检索。

...
| 方法 | 核心思想 | 局限性 |
|------|----------|--------|
| **DETR3D** | 3D query 投影回 2D 图像采样特征 | 投影误差、采样不全面、流程复杂 |
| **BEV 类方法** | 显式将多视图特征转为 BEV 表示 | 易引入 Z 轴误差，适配 3D 车道线等任务效果差 |

### 1.2 核心问题

**问题**：如何在避免 2D→3D 投影与特征采样的前提下，实现端到端多视图 3D 检测？

### 1.3 核心动机

受**隐式神经表示（INR）**启发，将**3D 坐标信息编码进 2D 特征**，生成 3D 位置感知特征，让目标查询直接在 3D 空间交互，规避 2D→3D 投影与特征采样。

---

## 2. 一句话总结

PETR 通过**3D 位置嵌入变换**，将多视图 2D 图像特征转化为 3D 位置感知特征，让目标查询直接在 3D 空间交互，实现无复杂投影采样的端到端多视图 3D 目标检测，在 nuScenes 数据集达 SOTA（50.4% NDS、44.1% mAP）。

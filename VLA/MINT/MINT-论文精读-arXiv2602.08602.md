...
设 $\mathbf{A} \in \mathbf{R}^{H \times D}$ 为一条动作序列，其中 $H$ 是时域长度，$D$ 是动作维度。

动作编码器 $\mathcal{E}$ 将输入序列映射为压缩潜在嵌入：

$$
f = \mathcal{E}(\mathbf{A}), \quad f \in \mathbf{R}^{L \times C}
$$
...
其中 $\mathbf{F} \in \mathbf{R}^{H \times D}$ 是频域表示。
...
其中每个 $\mathbf{s}_k \in \{1, \dots, V\}^{l_k}$ 是尺度 $k$ 的离散 token 图，$l_k$ 是该尺度的分辨率。设 $\mathcal{Z} \in \mathbf{R}^{V \times C}$ 为包含 $V$ 个码向量的共享码书。
...
#### A.6.1 公式维度一致性
- [ ] 所有矩阵/向量维度标注与正文描述一致（如 $\mathbf{A} \in \mathbf{R}^{H 	imes D}$）

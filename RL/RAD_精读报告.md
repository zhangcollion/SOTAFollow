...
端到端自动驾驶的目标是学习一个策略 $\pi_\theta(a_{t} | s_{t})$，直接从多视角图像序列映射到车辆控制动作，实现"感知-规划-控制"的统一建模。

然而，现有端到端自动驾驶方法主要采用**模仿学习（Imitation Learning, IL）**范式，在实际部署中面临根本性挑战。

### 1.2 相关工作与存在的问题

#### 问题一：因果混淆（Causal Confusion）

IL 捕获的是**相关性而非因果关系**。模型观察到驾驶场景中的视觉模式（如前方车辆位置）与人类动作之间的统计关联，但这种关联并不等同于因果推理。

后果：
- **Shortcut Learning**：模型学到的是"看起来像什么就怎么开"，而非"为什么这样开"
- 在训练分布内表现良好，但在分布外（out-of-distribution）场景中泛化能力差
- 无法处理训练时未见过但逻辑上应相通的场景

#### 问题二：开环训练-闭环部署 gap

现有方法多采用**开环训练**：模型在专家轨迹监督下学习从状态到动作的映射，但训练时并未真正在闭环环境中交互。

后果：
- **误差累积**：开环训练时每个时间步的微小预测误差不会影响下一步的输入（因为输入是真实状态），但闭环部署时这些误差会传播并放大
- **因果混淆加剧**：模型习惯于接收完美的感知输入，缺乏在闭环反馈中恢复的能力

#### 问题三：缺乏真实世界的试错学习

真实驾驶涉及大量**安全关键的罕见场景**（如紧急变道、突发障碍物）。这些场景在专家数据集中出现的频率极低，IL 方法无法有效学习如何应对。

### 1.3 本文动机

RAD 旨在通过**大规模闭环强化学习（RL）**解决上述问题：

1. **3DGS 数字孪生环境**：构建照片级真实的仿真环境，支持策略的大规模探索与试错学习，无需真实事故风险

2. **RL + IL 协同优化**：RL 补齐 IL 的因果建模能力和开环 gap，IL 补齐 RL 的人类对齐能力

3. **解耦离散动作空间**：将动作解耦为横向+纵向，缩短时域 horizon，降低探索复杂度，加速收敛
...
端到端自动驾驶的目标是学习一个策略 $\pi_\theta(a_{t} | s_{t})$，直接从多视角图像序列映射到车辆控制动作。
...
其中 $V_{x}$ 和 $V_{y}$ 使用**独立的价值头**（网络参数 $\theta_{V_{x}}$ 和 $\theta_{V_{y}}$），分别估计横向和纵向状态价值。
...
其中横向奖励 $r_{t}^{\text{lat}}$ 和纵向奖励 $r_{t}^{\text{lon}}$ 分别用于横向和纵向动作的价值估计和优势计算。
...
其中 $\rho_{t}^{\text{lat}} = \frac{\pi_{\theta}(a_t^{\text{lat}}|s_t)}{\pi_{\theta_{\text{old}}}(a_{t}^{\text{lat}}|s_{t})}$ 为重要性采样比率，$\epsilon_{\text{lat}}, \epsilon_{\text{lon}}$ 为裁剪阈值。

...
机器人操作任务面临的核心挑战是**数据稀缺**：获取大量高质量的人体演示轨迹成本极高，而现有方法往往需要数百甚至数千条演示才能达到可接受的性能。如何在极低数据量（5-10 条 demos）下完成复杂操作任务，是一个关键问题。

### 1.2 现有方法的局限性

| 方法类型 | 代表工作 | 核心问题 |
|---------|---------|----------|
| **2D 视觉 + 图文预训练** | BC-R3M, Diffusion Policy | 依赖静态图文对预训练，无法充分建模环境动态 |
| **3D 建模方法** | DP3, π₀.₅ | 缺乏时序建模能力，无法预测连续动作序列 |
| **视频预测方法** | UniPi, DreamZero | 仅预测热力图，忽略环境动态的联合建模 |

### 1.3 核心问题

**机器人操作需要同时理解两个维度：**
1. **3D 空间结构**：理解物体、工作空间的三维几何关系
2. **时序演化**：预测动作执行后环境如何变化

现有方法难以同时兼顾这两个维度：2D 图文预训练缺乏空间理解，3D 方法缺乏时序预测能力，视频预测方法（如 DreamZero）虽然利用视频基础模型，但仅预测热力图而忽略环境动态的联合建模。

### 1.4 MV-VDP 的核心出发点

MV-VDP 的核心洞察是：**视频基础模型学习到的时空动态建模能力可以直接迁移到机器人操作**。通过将点云正交投影为多视角 RGB 图、将末端执行器姿态编码为热力图，用视频扩散模型联合预测未来 RGB 视频和热力图序列，实现：
- RGB 视频建模环境动态
- 热力图视频编码动作信息
- 两者联合预测，实现数据高效的动作学习
...
- DreamZero（同期工作，相同 backbone）：待补充

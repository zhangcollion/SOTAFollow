...
### 1.2 Imagine-then-Execute 范式的困境
...
Fast-WAM 证明 WAMs 的核心价值在于训练阶段的视频联合建模（video co-training），而非推理阶段显式生成未来观察。移除 video co-training 造成的性能损失远大于移除 test-time 未来想象的损失。

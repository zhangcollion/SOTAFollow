...
联合嵌入预测架构（JEPA）因其高效的特征学习能力而备受关注，但在实际应用中，现有方法存在严重问题：

| 缺陷 | 具体表现 | 影响 |
|------|---------|------|
| **多 term loss** | 6个独立调参项（如 PLDM），训练过程脆弱 | 超参数搜索困难，训练不稳定 |
| **依赖 EMA** | 需要指数移动平均维持表示稳定性 | 增加计算开销和训练复杂度 |
| **预训练编码器** | 受限的预训练知识天花板 | 无法端到端优化，无法适应新任务 |
| **像素级重建** | 计算昂贵的图像重建 | 关注视觉细节而非语义表征 |
| **依赖 Reward** | 需要显式奖励信号 | 限制了在无奖励环境中的应用 |

### 1.2 表示崩溃问题

表示崩溃（Representation Collapse）是端到端 JEPA 训练的核心挑战：当编码器将所有输入映射到同一常量向量时，预测损失达到最小，但模型完全丧失区分能力。

现有方法通过复杂的防崩溃机制解决这一问题（如多 term loss、EMA、对比学习），但这些机制本身引入了新的超参数负担，偏离了"简洁端到端学习"的初衷。

### 1.3 LeWorldModel 的核心出发点

**问题本质**：latent 分布缺乏约束——没有机制保证学到的表示是有意义的。

**解决思路**：用统计学中**全部分布匹配**（而非逐点或边缘匹配）的思路，强制 latent embedding 服从高斯分布。通过 SIGReg 正则器，只需一个超参数 λ，就能从根本上消除崩溃，实现真正端到端的 JEPA 训练。
...
其中 $Z = \{z_{1}, z_{2}, \ldots, z_N\}$ 是所有时间步的 embedding 集合。
...
| 符号 | 含义 |
|------|------|
| $z_{t} = \text{enc}_\theta(o_{t})$ | 图像 $o_{t}$ 的 latent embedding |
| $\hat{z}_{t+1} = \text{pred}_\phi(z_{t}, a_{t})$ | 基于动作 $a_{t}$ 预测的下一帧 embedding |
| $\mathcal{L}_{\text{pred}} = \|\hat{z}_{t+1} - z_{t+1}\|_{2}^{2}$ | Prediction loss |
